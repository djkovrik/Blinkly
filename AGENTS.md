...
- `shared/crashlytics` - GitLive Firebase Crashlytics implementation of the shared crash-reporting contract
...
- `shared/domain` - business interfaces, models, core logic, and the platform-neutral typed analytics contract
- `shared/notifier` - notification permissions and notification API implementation
- `shared/settings` - settings storage through Multiplatform Settings, including the device-local analytics opt-out and pre-SDK installation bootstrap state
...
- Native Firebase Auth owns secure session persistence; never persist Google/Firebase credentials, tokens, or UID in Blinkly storage. Sync authorization is exposed as a three-phase `StateFlow`: `Restoring` until the first native auth-listener result, `SignedIn` for a restored or newly confirmed Firebase user, and `SignedOut` only after the listener confirms `null`. A synchronous persisted `currentUser` may optimistically seed `SignedIn`, but an initialization-time `null` must never be treated as sign-out by a Store or UI. Google Sign-In callbacks must match the native Firebase user's UID before sync starts.
...
- Yandex advertising is configured at the Android/iOS entry points and exposed only through a safe-disabled Compose `CompositionLocal`; it is not component or Store state. Both Achievements and Garden use contextual adaptive inline banners as regular lazy-list items. Android removes `AD_ID`, iOS does not request ATT/IDFA, and ad requests must contain only the platform-specific `adUnitId`, never Blinkly health, workout, reminder, account, achievement, garden, or sync data. Yandex Mobile Ads 8.1.0 transitively packages AppMetrica on both platforms; keep those required transitive artifacts, but never activate AppMetrica or report Blinkly product events through it. The iOS `Podfile` must link YandexMobileAds together with the native FirebaseAnalytics, FirebaseAuth, FirebaseCrashlytics, FirebaseFirestore, and GoogleSignIn SDKs required by the app and GitLive/KMPAuth, using `use_frameworks! :linkage => :static`; do not replace this with dynamic linkage or global `use_modular_headers!`. Yandex documents Xcode 16.4 as the minimum for YandexMobileAds 8.1.0, but the Intel Simulator `x86_64` slice requires Xcode 26.1.1 or newer in practice because Xcode 16.4 lacks `_swift_coroFrameAlloc`; Intel Macs that cannot install that toolchain must use a supported Mac or macOS CI runner for iOS builds.
- Firebase Analytics is configured only by the Android/iOS app entry points and exposed to shared code through `BlinklyAnalyticsReporter`; components receive the typed `BlinklyAnalytics` facade from `DomainModule`. Debug builds keep Analytics deactivated, while Release builds apply the device-local analytics preference before creating the root component. Keep the iOS Analytics pod on `FirebaseAnalytics/WithoutAdIdSupport` while Firebase remains on 11.x; Firebase 12 replaces that subspec with `FirebaseAnalytics/Core`. Firebase Crashlytics remains the only crash reporter.
- Firebase Analytics is enabled by default and controlled by the device-local `BlinklySettings.analyticsEnabled` preference. Do not include this preference in sync snapshots or advance sync change timestamps when it changes. The Preferences Store persists the setting and then updates the active reporter. This option controls only Firebase Analytics; it must not disable or reconfigure Firebase Auth, Firestore, Crashlytics, or Yandex Mobile Ads.
- Firebase Analytics event names and parameters are an allow-listed contract in `BlinklyAnalyticsEvent`. Emit only low-cardinality product funnel enums owned by the component/Store where the action succeeds. Never send account/profile IDs, email, reminder dates/times/content, exercise details, health data, achievement/garden state, location, free text, exceptions, stack traces, credentials, or stable custom user/device IDs. Keep Advertising ID, IDFA, IDFV, and advertising-personalization signals disabled, and do not set Firebase Analytics user IDs or user properties without a separate product/privacy decision.
...
- GitLive Firebase 2.5 Android dependencies must be aligned with the explicit Firebase BOM in `shared/compose`, `shared/crashlytics`, and `shared/component/sync`. `shared/compose` also declares the GitLive app/auth artifacts directly so KMPAuth's older transitive pin cannot select a mixed GitLive version.
- Crashlytics uses `BlinklyCrashReporter` from domain external and the GitLive-backed `shared/crashlytics` implementation. `RootComponentDefault` records caught component errors as non-fatal reports while native Firebase SDKs capture fatal crashes. GitLive does not replace platform setup: Android must apply the Crashlytics Gradle plugin and package the native SDK; iOS must link the `FirebaseCrashlytics` pod, generate dSYMs for every configuration, and keep the Crashlytics symbol-upload script as the final app-target build phase. Never attach health, workout, reminder, account, achievement, garden, sync, advertising, email, or other personally identifying data to Crashlytics logs or custom keys. Set a Crashlytics user ID only after an explicit product/privacy decision.
- AGP 9 strict R8 full mode can remove the reflectively invoked `CrashlyticsRegistrar` constructor and leave `FirebaseCrashlytics.getInstance()` failing with `FirebaseCrashlytics component is not present` in minified releases. Retain `android.r8.strictFullModeForKeepRules=false` until the Firebase Android SDK fixes this compatibility issue and a signed minified release verifies initialization. Treat crash reporting as best-effort: a reporter failure must be logged and must never replace or suppress the original component error.
- In CI, do not link standalone `iosSimulatorArm64Test` executables for `shared/crashlytics`, `shared/component/sync`, `shared/component/root`, or `shared/compose`: their GitLive/KMPAuth native dependencies require Firebase and GoogleSignIn framework search paths supplied by the CocoaPods Xcode workspace. Run the remaining shared iOS tests through Gradle, keep common coverage for those modules in the Android test job, link the static Compose framework separately, and verify native SDK integration by building `iosApp/iosApp.xcworkspace` after `pod install`.
...
- database, notifier, alarm, settings, beeper, crashlytics, utils -> implementations of those external interfaces
...
- `shared/crashlytics/src/commonMain/kotlin/com/sedsoftware/blinkly/crashlytics/di/CrashlyticsModule.kt`
...
`RootComponentFactory` is the composition root on both platforms. It builds dispatchers, utils, platform modules (`alarm`, `database`, `notifier`, `settings`, `beeper`, `crashlytics`), the platform `BlinklyScreenAwakeController`, and the native Firebase Analytics-backed `BlinklyAnalyticsReporter`, then `SyncModule` tracking wrappers, then `DomainModule`, then creates `BlinklySyncManager` with the domain reminder reschedule callback before `RootComponentDefault`. `DomainModule` creates the typed analytics facade with the initial device-local setting and injects it only into components that own tracked actions. The Swift-facing analytics reporter and bootstrap-state types are declared in `shared/compose` and adapted there to domain/settings types; do not expose types from non-exported framework dependencies directly in the Compose framework API.
...
`RootComponentDefault` maps incoming `ComponentOutput.Common.ErrorCaught` values to `BlinklyError`, logs the original cause, records it through `BlinklyCrashReporter` as a non-fatal Crashlytics report, and emits the error through `RootComponent.errors`.
