...
- Sync authorization state is seeded from Firebase Auth's persisted `currentUser` and exposed as a `StateFlow`; new sync Stores must initialize from its current value so reopening Preferences never falls back to a transient signed-out model.
