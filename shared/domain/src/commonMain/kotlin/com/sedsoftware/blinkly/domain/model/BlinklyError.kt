...
sealed class BlinklyError(message: String, cause: Throwable? = null) : Exception(message, cause) {
    ...
    class ExactAlarmPermissionChecking(cause: Throwable) : BlinklyError("Unable to check exact alarm permission", cause)
    class ExactAlarmPermissionRequesting(cause: Throwable) : BlinklyError("Unable to request exact alarm permission", cause)
    class ExactAlarmPermissionDenied : BlinklyError("Exact alarms must be enabled in system settings")
    class RemindersRescheduling(cause: Throwable) : BlinklyError("Unable to reschedule reminders", cause)
