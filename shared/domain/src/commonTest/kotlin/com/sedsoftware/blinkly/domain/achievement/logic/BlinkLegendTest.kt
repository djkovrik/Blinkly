...
internal class BlinkLegendTest : BaseAchievementTest() {

    private val blinkBreakCount: Int = 60

    override val achievement: UnlockableAchievement = BlinkLegend(blinkBreakCount)

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.BLINK_BREAK, 334)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.BLINK_BREAK, 300)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
