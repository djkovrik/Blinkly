...
internal class BlinkStarterTest : BaseAchievementTest() {

    private val blinkBreakCount: Int = 60

    override val achievement: UnlockableAchievement = BlinkStarter(blinkBreakCount)

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.BLINK_BREAK, 2)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithFullAndPartialDays(now, 1)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
