...
internal class EveningNewbieTest : BaseAchievementTest() {

    private val palmingDuration: Int = 120

    override val achievement: UnlockableAchievement = EveningNewbie(palmingDuration)

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.PALMING, 4)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.PALMING, 1)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
