...
internal class ExpressMasterTest : BaseAchievementTest() {

    override val achievement: UnlockableAchievement = ExpressMaster()

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.TWENTY_X3, 51)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.TWENTY_X3, 45)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
