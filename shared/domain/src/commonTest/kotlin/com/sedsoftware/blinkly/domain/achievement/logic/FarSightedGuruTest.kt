...
internal class FarSightedGuruTest : BaseAchievementTest() {

    override val achievement: UnlockableAchievement = FarSightedGuru()

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.TWENTY_X3, 2001)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.TWENTY_X3, 1980)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
