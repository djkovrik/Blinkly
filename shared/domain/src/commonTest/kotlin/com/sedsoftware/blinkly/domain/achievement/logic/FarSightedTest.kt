...
internal class FarSightedTest : BaseAchievementTest() {

    override val achievement: UnlockableAchievement = FarSighted()

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.TWENTY_X3, 101)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.TWENTY_X3, 90)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
