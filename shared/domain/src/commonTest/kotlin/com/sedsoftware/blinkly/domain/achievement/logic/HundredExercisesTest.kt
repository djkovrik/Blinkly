...
internal class HundredExercisesTest : BaseAchievementTest() {

    override val achievement: UnlockableAchievement = HundredExercises()

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithFullAndPartialDays(now, 30)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithFullAndPartialDays(now, 10)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
