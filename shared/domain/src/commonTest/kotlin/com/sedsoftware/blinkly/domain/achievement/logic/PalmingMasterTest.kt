...
internal class PalmingMasterTest : BaseAchievementTest() {

    private val palmingDuration: Int = 120

    override val achievement: UnlockableAchievement = PalmingMaster(palmingDuration)

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.PALMING, 150)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.PALMING, 140)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
