...
internal class RelaxEnthusiastTest : BaseAchievementTest() {

    private val palmingDuration: Int = 120

    override val achievement: UnlockableAchievement = RelaxEnthusiast(palmingDuration)

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.PALMING, 30)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.PALMING, 20)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
