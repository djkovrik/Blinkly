...
internal class ReverseMyopeTest : BaseAchievementTest() {

    override val achievement: UnlockableAchievement = ReverseMyope()

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.NEAR_FAR_FOCUS, 50)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.NEAR_FAR_FOCUS, 25)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
