...
internal class TheAllSeeingTest : BaseAchievementTest() {

    override val achievement: UnlockableAchievement = TheAllSeeing()

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithFullDays(now, 7)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithFullDays(now, 7).toMutableList()
        calendar.removeAt(1)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
