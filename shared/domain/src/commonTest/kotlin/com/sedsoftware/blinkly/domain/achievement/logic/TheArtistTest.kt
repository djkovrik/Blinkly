...
internal class TheArtistTest : BaseAchievementTest() {

    private val figureEightCount: Int = 10

    override val achievement: UnlockableAchievement = TheArtist(figureEightCount)

    @Test
    fun `when calendar and achievements match logic then unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.FIGURE_EIGHT, 3)
        // when
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithMinExerciseCount(now, ExerciseType.FIGURE_EIGHT, 1)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
