...
internal class ThousandAndOneTest : BaseAchievementTest() {

    override val achievement: UnlockableAchievement = ThousandAndOne()
    ...
    fun `when calendar and achievements do not match logic then not unlocked`() = runTest {
        // given
        val calendar = FakeData.getCalendarWithFullDays(now, 109)
        // when
        val unlocked = achievement.unlocked(emptyAchievements, calendar)
        // then
